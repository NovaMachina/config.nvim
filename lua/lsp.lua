...
if not (mason_status_ok and mason_tool_installer_ok) then
	vim.api.nvim_echo(
		{ { "Mason, Mason Tool Installer, Completion, or LSP Format not installed!" } },
		true,
		{ err = true }
	)
	return
...
local all = concat(servers, linters, debuggers, formatters)
...
M.on_attach = function(client, bufnr)
	...
	if client.name == "json-lsp" and not is_lazy_lock then
